...
import { BellRing, Filter, PlusCircle, Printer, Search } from "lucide-react";
import { toast } from "sonner";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { DateRangePicker } from "@/shared/components/dateRangePicker/DateRangePicker";
import { usePTituloIntimacaoBatchReadHook } from "@/packages/intimacao-lote/hooks/PTituloIntimacaoBatch/usePTituloIntimacaoBatchReadHook";
...
function getStatusIntimacaoCode(item: PTituloIntimacaoBatchInterface): "P" | "I" {
  return item.data_intimacao ? "I" : "P";
}

function calculateSelectedTotal(data: PTituloIntimacaoBatchInterface[], selectedIds: Set<number>): number {
...
export default function PTituloIntimacaoBatchIndex() {
  const { titulosIntimacaoBatch, isLoading, fetchTitulosIntimacaoBatch } = usePTituloIntimacaoBatchReadHook();
  ...
  useEffect(() => {
    ...
    void fetchTitulosIntimacaoBatch();
  }, [fetchTitulosIntimacaoBatch]);
  ...
  const filteredData = useMemo(() => {
    ...
    return titulosIntimacaoBatch
      .filter((item) => Boolean(item.data_apontamento))
      .filter((item) => {
        if (query) {
          const searchContent = [
            item.numero_titulo,
        ...
        if (statusFilter) {
          if (getStatusIntimacaoCode(item) !== statusFilter) return false;
        }
  ...
  }, [searchQuery, statusFilter, dateRange?.from, dateRange?.to, titulosIntimacaoBatch]);

  const resumo = useMemo(() => {
    const total = filteredData.length;
    const pendentes = filteredData.filter((item) => getStatusIntimacaoCode(item) === "P").length;
    const intimados = filteredData.filter((item) => getStatusIntimacaoCode(item) === "I").length;
    ...
    return { total, pendentes, intimados, selecionados, valorSelecionado };
  ...
  const handlePrintSelectedIntimacoes = () => {
    ...
    if (notIntimated.length > 0) {
      toast.warning("Impressão bloqueada", {
        description:
          "Existem títulos selecionados sem intimação emitida. Remova-os da seleção para imprimir em lote.",
      });
      return;
    }
    ...
    <div className="flex w-full flex-col gap-5">
      <section className="rounded-xl border bg-card p-4 shadow-xs md:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Organize e dispare intimações em massa para os títulos já apontados.
            </p>
          </div>
          <div className="flex w-full flex-col items-stretch gap-2 sm:w-auto sm:flex-row sm:items-center sm:justify-end">
            ...
            <Button
              ...
              onClick={handlePrintSelectedIntimacoes}
            >
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm">
            Já intimados: <span className="font-semibold text-emerald-700">{resumo.intimados}</span>
          </div>
          <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm">
      ...
      <section className="rounded-xl border bg-card p-4 shadow-xs md:p-5">
        <div className="grid gap-4 xl:grid-cols-[1fr_auto]">
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Status intimação</label>
              <Select value={statusFilter || "all"} onValueChange={(value) => setStatusFilter(value === "P" || value === "I" ? value : "")}>
                <SelectTrigger>
