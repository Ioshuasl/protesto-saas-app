...
import {
  pTituloRetiradaFormSchema,
  type PTituloRetiradaFormValues,
} from "@/packages/administrativo/schemas/PTitulo/PTituloRetiradaFormSchema";
...
export interface PTituloRetiradaFormProps {
  titulo?: Partial<TituloListItem> | Partial<PTituloInterface>;
  defaultValues?: Partial<PTituloRetiradaFormValues>;
  onSubmit?: (values: PTituloRetiradaFormValues) => void;
  id?: string;
  className?: string;
  children?: ReactNode;
}

export function PTituloRetiradaForm({
  titulo,
  defaultValues,
  onSubmit,
  id,
  className,
  children,
}: PTituloRetiradaFormProps) {
  const resolvedDefaults = useMemo((): PTituloRetiradaFormValues => {
    const dataRetirada = defaultValues?.data_retirada ?? getTodayLocalIsoDateOnly();
  ...
  const form = useForm<PTituloRetiradaFormValues>({
    resolver: zodResolver(pTituloRetiradaFormSchema),
    mode: "onChange",
