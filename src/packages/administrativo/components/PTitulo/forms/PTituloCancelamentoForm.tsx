...
import {
  pTituloCancelamentoFormSchema,
  type PTituloCancelamentoFormValues,
} from "@/packages/administrativo/schemas/PTitulo/PTituloCancelamentoFormSchema";
...
export interface PTituloCancelamentoFormProps {
  titulo?: Partial<TituloListItem> | Partial<PTituloInterface>;
  defaultValues?: Partial<PTituloCancelamentoFormValues>;
  onSubmit?: (values: PTituloCancelamentoFormValues) => void;
  id?: string;
  className?: string;
  children?: ReactNode;
}

export function PTituloCancelamentoForm({
  titulo,
  defaultValues,
  onSubmit,
  id,
  className,
  children,
}: PTituloCancelamentoFormProps) {
  const resolvedDefaults = useMemo((): PTituloCancelamentoFormValues => {
    const dataCancelamento =
      defaultValues?.data_cancelamento ??
      tituloDateFieldToIso(titulo?.data_cancelamento) ??
      getTodayLocalIsoDateOnly();
  ...
  const form = useForm<PTituloCancelamentoFormValues>({
    resolver: zodResolver(pTituloCancelamentoFormSchema),
    mode: "onChange",
