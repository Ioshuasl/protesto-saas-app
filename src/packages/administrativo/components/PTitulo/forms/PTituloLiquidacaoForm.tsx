...
import {
  pTituloLiquidacaoFormSchema,
  type PTituloLiquidacaoFormValues,
} from "@/packages/administrativo/schemas/PTitulo/PTituloLiquidacaoFormSchema";
...
export interface PTituloLiquidacaoFormProps {
  titulo?: Partial<TituloListItem> | Partial<PTituloInterface>;
  defaultValues?: Partial<PTituloLiquidacaoFormValues>;
  onSubmit?: (values: PTituloLiquidacaoFormValues) => void;
  id?: string;
  className?: string;
  children?: ReactNode;
}

export function PTituloLiquidacaoForm({
  titulo,
  defaultValues,
  onSubmit,
  id,
  className,
  children,
}: PTituloLiquidacaoFormProps) {
  const resolvedDefaults = useMemo((): PTituloLiquidacaoFormValues => {
    const dataLiquidacao =
      defaultValues?.data_liquidacao ??
      tituloDateFieldToIso(titulo?.data_pago) ??
      getTodayLocalIsoDateOnly();
  ...
  const form = useForm<PTituloLiquidacaoFormValues>({
    resolver: zodResolver(pTituloLiquidacaoFormSchema),
    mode: "onChange",
