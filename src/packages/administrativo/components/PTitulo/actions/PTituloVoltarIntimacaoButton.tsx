...
import { usePTituloVoltarIntimacaoHook } from "@/packages/administrativo/hooks/PTitulo/usePTituloVoltarIntimacaoHook";
...
import {
  pTituloVoltarIntimacaoSchema,
  type PTituloVoltarIntimacaoFormValues,
} from "@/packages/administrativo/schemas/PTitulo/PTituloVoltarIntimacaoSchema";
import ConfirmDialog from "@/shared/components/confirmDialog/ConfirmDialog";
import { cn } from "@/lib/utils";

export function PTituloVoltarIntimacaoButton({ id, onSuccess }: { id: number; onSuccess: (titulo: TituloListItem) => void }) {
  const { voltarParaIntimacao } = usePTituloVoltarIntimacaoHook();
  const [isLoading, setIsLoading] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const [isConfirmDialogOpen, setIsConfirmDialogOpen] = useState(false);

  const form = useForm<PTituloVoltarIntimacaoFormValues>({
    resolver: zodResolver(pTituloVoltarIntimacaoSchema),
    mode: "onChange",
    defaultValues: {
      ocorrencia_id: 0,
    },
  ...
  const buildVoltarIntimacaoPayload = (
    values: PTituloVoltarIntimacaoFormValues,
  ): PTituloVoltarIntimacaoInterface => {
    return {
      ocorrencia_id: Number(values.ocorrencia_id),
    };
  };

  const handleConfirm = async (values: PTituloVoltarIntimacaoFormValues) => {
    const payload = buildVoltarIntimacaoPayload(values);
    setIsLoading(true);
    try {
      const response = await voltarParaIntimacao(id, payload);
    ...
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>Voltar para Intimação</DialogTitle>
            <DialogDescription>
              Selecione a ocorrência para confirmar o retorno do título para intimação.
            </DialogDescription>
          </DialogHeader>
      ...
      <ConfirmDialog
        ...
        title="Voltar para Intimação"
        description="Confirmação obrigatória"
        message="Confirma o retorno deste título para intimação com a ocorrência informada?"
