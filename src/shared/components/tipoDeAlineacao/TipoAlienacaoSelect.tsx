...
import { TipoAlienacaoEnum } from '@/shared/enums/TipoAlienacao';

import TipoAlienacaoSelectInterface from './interface/TipoAlienacaoSelectInterface';

export default function TipoAlienacaoSelect({ field }: TipoAlienacaoSelectInterface) {
  const [open, setOpen] = React.useState(false);

  const options = Object.entries(TipoAlienacaoEnum).map(([key, label]) => ({
    value: Number(key),
    label,
  }));
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
        <Command>
          <CommandInput placeholder="Buscar tipo de alienação..." className="w-full" />
